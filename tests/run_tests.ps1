...
try {
    ...
    # Compile the full firmware entry and the always-on apps in the default
    # (balance simple + drive demo) configuration to catch missing symbols.
    foreach ($Source in @("src/main.c", "src/app/button_app.c",
        ...
        $Object = Join-Path $TestBuildDir "$($SourceName)_default.o"
        & gcc @CommonFlags -c $Source -o $Object
        if ($LASTEXITCODE -ne 0) {
            throw "$Source default compile failed"
        }
    }
