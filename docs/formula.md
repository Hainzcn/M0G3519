...
本文统一车辆运动规划、加速度 IMU 修正、滚球控制、摆杆连杆反解、Emm42 步进电机控制和停车控制使用的公式。所有三角函数使用弧度，程序实现前必须统一坐标方向和时间戳。
...
$$
CB=21,\qquad CD=15.5,\qquad DP=3.5,\qquad BP=4.6
$$

摆杆轴高 $CF=5.2$，步进电机轴高为 $4.7$。以摆杆轴心 $C$ 为原点、车头方向为 $+x$、竖直向上为 $+y$，并将 $CD=15.5$ 作为两轴的水平距离，则：

$$
C=(0,0),\qquad D=(15.5,4.7-5.2)=\boxed{(15.5,-0.5)}
$$

控制器给出目标摆杆角 $\alpha$ 后，摆杆连杆孔坐标为：

$$
B=(21\cos\alpha,\ 21\sin\alpha)
$$
...
\gamma(\alpha)=
\operatorname{atan2}
\left(21\sin\alpha+0.5,\ 21\cos\alpha-15.5\right)
}
$$

代入 $DP=3.5$、$BP=4.6$ 后，步进电机曲柄目标角可直接计算为：
...
\frac{\rho(\alpha)^2-8.91}{7\rho(\alpha)},
-1,1
\right)
\right],\qquad s_\theta\in\{-1,+1\}
}
$$

其中 $s_\theta$ 必须按实物装配分支选定，运行中不能切换。计算反余弦前必须先检查机构是否可达：

$$
\boxed{1.1\le\rho(\alpha)\le8.1}
$$

摆杆水平时 $\alpha=0$，有 $\rho(0)=\sqrt{30.5}\approx5.523\text{ cm}$，位于可达范围内。若 Emm42 默认每圈 $3200$ 脉冲，则相对水平标定曲柄角 $\theta_0$ 的指令为：

$$
\boxed{
\theta_{deg,cmd}=\frac{180}{\pi}\theta_{cmd},\qquad
n=\operatorname{round}\left[
\frac{3200}{2\pi}(\theta_{cmd}-\theta_0)
\right]
}
$$

以上长度可以统一使用厘米，因为几何逆解中只出现长度比；$\alpha$、$\theta_{cmd}$ 和 $\theta_0$ 均使用弧度。
...
- 曲柄角度 $\theta$：步进电机输出轴曲柄向量 $\overrightarrow{DP}$ 相对车体 $+x_B$ 轴的角度，逆时针为正。
...
| $\theta$ | 步进电机曲柄相对车体水平面的角度 | rad |
| $\theta_{deg}$ | 下发给 Emm42 的目标角 | deg |
| $n$ | Emm42 相对当前位置的脉冲增量 | pulse |
...
$d_x$ 汇总滚动阻力、车体俯仰、步进电机振动、摆杆角运动和模型误差。
...
- $\tau_{dead}$：位置指令发出到曲柄开始运动的通信与驱动器响应延迟。
- $\tau_{rise}$：正式负载下小角度阶跃的主要上升时间（含 Emm42 位置环）。
- $\tau_{schedule}$：控制计算到 UART 帧真正发出之间的平均延迟。

这些参数必须使用 Emm42、正式连杆、正式摆杆和正式供电实测。3519 在时刻 $t$ 使用 $a_{plan}(t+\tau_{ff})$，使摆杆与车辆加速同步，而不是等 IMU 检测到加速后才动作。
...
图中的 $CP$ 是随步进电机曲柄转动而改变方向的辅助线，并不是固定水平线。
...
程序中控制器先得到目标摆杆角 $\alpha$，再直接求曲柄角 $\theta$。
...
若工作点 $|J|$ 很小，机构接近死点，步进电机需要很大的角位移和角速度才能改变摆杆角。全部工作范围应远离 $J=0$。

## 7. Emm42 步进电机角到位置指令

项目采用 Emm42 闭环步进驱动，经 UART7（PA23/PA24）接收 3519 的位置/速度指令。驱动器内部完成电流环和位置环，3519 不直接输出 PWM 脉宽。

默认细分下每圈脉冲数为 $N_{rev}=3200$。角度与脉冲关系：
...
\operatorname{round}
\left(
N_{rev}\frac{\theta_{cmd}-\theta_0}{2\pi}
\right)
}
$$

$\theta_0$ 是摆杆机械水平时的曲柄零位，上电后需通过回零或人工标定写入 Emm42 当前位置。
...
\boxed{
\theta_{deg,cmd}=
\frac{180}{\pi}\theta_{cmd}
}
$$

下发时使用 Emm42 位置模式，并限制最大转速 $\omega_{max}$ 和加速度 $a_{max}$：

$$
\boxed{
\text{emm42\_move\_angle}
\left(
\theta_{deg,cmd},
\omega_{max},
a_{max},
\text{relative/absolute}
\right)
}
$$

更推荐使用双向标定查表，补偿机构回差和方向性误差：
...
\left[
\operatorname{sat}(\theta_{deg,out},\theta_{min},\theta_{max}),
\dot{\theta}_{max}
\right]
$$

摆杆控制环默认与滚球控制同为 200Hz；Emm42 位置指令刷新率应不低于 100Hz，并实测 UART 发送、驱动器响应和负载下的跟随误差。步进电机与驱动器使用独立大电流电源，并与 3519 共地。
...
8. 用 Emm42 双向标定表计算 $\theta_{deg,cmd}$，执行角度和变化率限制。
9. 按已验证的 100~200Hz 周期通过 UART7 更新 Emm42 位置指令。
10. 记录 $x,u,\hat a_c,\alpha_{cmd},\theta_{cmd},\theta_{deg,cmd}$ 和所有饱和标志。
...
| $\theta_0,\theta_{min},\theta_{max}$ | Emm42 台架标定 |
| $N_{rev}$ | 驱动器细分设置，默认 3200 pulse/rev |
| $LUT_{up},LUT_{down}$ | 双方向逐点测量摆杆角 |
| $\tau_{dead},\tau_{rise}$ | 位置指令事件标记与高速录像或角度传感器 |
| $\omega_{max},a_{max}$ | 负载下允许的最大转速和加速度 |
...
所有参数必须连同测试日期、供电电压、机构版本和固件版本保存。更换曲柄孔位、连杆长度、摆杆位置或 Emm42 细分/供电后，需要重新检查几何映射和动态延迟。

## 12. 模型适用范围

公式负责提供正确方向和主要提前量，下列误差最终由传感器修正和视觉反馈消除：

- Emm42 跟随误差、加减速限制、通信丢帧和负载下重复定位误差。
...
底盘已按第 8 节开始实现 100 Hz 双层控制框架：红外加权误差经循迹 PID 生成左右轮目标 RPM，每侧轮速 PID 叠加 $k_S/k_V/k_A$ 前馈后输出 PWM。代码结构、启用接口、安全策略和调参顺序见 [循迹两驱PID框架.md](循迹两驱PID框架.md)。
