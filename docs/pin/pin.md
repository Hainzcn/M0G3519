...
| STBY / VCC | 3.3V | - | 逻辑电源 |
| VM | 5~12V | - | 电机电源 |
| ADC | A26 (ADC0_CH1) | 模拟输入 | 带稳压版驱动板的 VM 分压输出（VM/11） |
...
轮趣带稳压版的 `ADC` 引脚不是数字通信接口，而是电机电源 `VM` 经 `100kΩ/10kΩ` 分压后的模拟电压，比例为 `1/11`。本工程将该引脚接到 A26（ADC0_CH1），以 12 位 ADC 读取：

`VM_mV = raw / 4095 × 3300 × 11`

`src/middle/power.c` 每 20 ms 采样，单次包含 16 次均值，并做 1/4 低通滤波。UART0 每 200 ms 输出 `[pwr] <ms>,ok=<0|1>,mv=<mV>,raw=<ADC>,e=<错误累计>`；`ok=0` 或 `e` 持续增加说明 ADC 未完成转换，优先检查 A26、ADC 引脚和共地。采集串口日志后运行 `python tools/plot_bus_voltage.py <日志文件> --output bus_voltage.svg` 可生成电压曲线。
...
模块为 I2C 外设，7 位地址 `0x5C`。主控读取寄存器 `5` 获取六路数字状态；模块内部完成红外采样和学习校准。

| 模块信号 | MCU 引脚 | 方向 | 说明 |
| --- | --- | --- | --- |
| SCL | B0 (PB0) | I2C0 SCL | 与 OLED 共用总线 |
| SDA | B1 (PB1) | I2C0 SDA | 与 OLED 共用总线 |
| 5V | 5V | — | 传感器额定供电 |
| GND | GND | — | 与 MCU 共地 |

I2C 上拉高电平必须不超过 MCU 的 3.3V I/O 电平；若传感器将 SDA/SCL 上拉到 5V，必须串接双向电平转换器。

读取逻辑见 `src/hardware/grayscale_hw.c`（I2C 寄存器读取）与 `src/middle/grayscale.c`（5 ms 周期扫描）。读取前会避让 OLED 的 I2C 传输，控制层只使用完整的一帧六路状态。

串口调试：上电后除 `[hb]` 心跳外，每 1 s 输出一路 `[gs]` 报文，格式为：

`[gs] <序号>,ok=<0|1>,v=<v0><v1><v2><v3><v4><v5>,e=<I2C错误累计>`

`ok=1` 表示最近一次主循环获得了新的稳定帧；`e` 持续增长说明地址、接线或 I2C 电平存在问题。示例：`[gs] 12,ok=1,v=001111,e=0`。

其中 `v0`~`v5` 对应 X1~X6 探头，值为 0 或 1（厂家说明：灯亮/检测到为 1）。传感器联调阶段建议将 `motor_app.c` 中 `MOTOR_APP_DEMO_ENABLE` 设为 `0`，避免电机转动干扰读数。
