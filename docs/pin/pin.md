...
> 完整引脚分配速查表见 [pin_table.md](pin_table.md)。

# 当前 IO 资源审计（2026-08-05）

核心板两侧排针共引出 **54 个 GPIO**。按当前固件和接线统计，加入蜂鸣器后有 28 个引脚用于外设或用户按键，PA18 固定保留为 BSL 启动采样脚，剩余 **25 个可安全用作普通 GPIO**。

| 功能 | 已占用引脚 |
| --- | --- |
| 电机 PWM / 方向 | PA0、PA1、PB2、PB3、PB4、PB5 |
| 左右轮 QEI | PB10、PB11、PB7、PB9 |
| UART0 / UART1 / UART3 / UART7 | PA10、PA11、PA8、PA9、PB12、PB13、PA23、PA24 |
| 红外 I2C1 / OLED I2C0 | PA15、PA16、PB17、PB18 |
| 用户按键 SW1~SW4 | PA30、PA31、PB1、PB0 |
| 板载 LED / 有源蜂鸣器 | PA14、PA13 |
| 板载 BSL 按键（保留） | PA18 |

| 剩余类别 | 引脚 |
| --- | --- |
| A 口安全空闲 GPIO（11 个） | PA2、PA7、PA12、PA17、PA21、PA22、PA25、PA26、PA27、PA28、PA29 |
| B 口安全空闲 GPIO（14 个） | PB6、PB8、PB14、PB15、PB16、PB19、PB20、PB21、PB22、PB23、PB24、PB25、PB26、PB27 |

“安全空闲”表示当前工程未初始化且核心板上无固定负载；若改作定时器、UART、I2C 等复用功能，仍须检查对应外设实例是否已被占用。PA18 虽可配置为 GPIO，但上电时参与 BSL 判断，不纳入可用清单。PA19/PA20 为 SWDIO/SWCLK，且未引到两侧排针。
...
| 信号 | MCU 引脚 | 说明 |
| --- | --- | --- |
| 控制信号 | A13 (PA13) | 普通推挽 GPIO，默认高电平有效 |
| 电源 | 按模块额定值接 3.3V 或 5V | 禁止由 GPIO 引脚直接给蜂鸣器供电 |
| GND | GND | 必须与 MCU 共地 |

任务正常完成后播放 3 次短鸣，每次响 120 ms、间隔 100 ms。提示音由主循环中的非阻塞状态机产生，不占用 PWM 定时器，也不会暂停底盘、平衡或通信任务。超时、故障和 SW4 人工停止不触发完成提示。

若使用带 `VCC/GND/IN` 的有源蜂鸣器模块，`IN` 接 PA13；低电平触发模块需将 `buzzer_hw.h` 中 `BUZZER_HW_ACTIVE_LEVEL` 改为 `0u`。若使用两脚裸蜂鸣器，不应直接接 GPIO：建议用 NPN 三极管或逻辑级 NMOS 低端驱动，GPIO 经 1~4.7 kΩ 电阻接基极/栅极，蜂鸣器接额定电源；磁性蜂鸣器还应并联续流二极管。

有源蜂鸣器内部自带振荡器，只能控制响/停，当前“三短鸣”是节奏提示；如需不同音高的旋律，需更换无源蜂鸣器并分配 PWM 通道。
...
原灰度模块的 AD0/AD1/AD2/OUT 接线已停用；A12 释放，A13 现用于有源蜂鸣器，A15/A16 改作红外模块 I2C1。模块虽然按官方说明使用 5V 供电，但 **SDA/SCL 高电平必须实测不超过 3.3V**；若模块板将 I2C 上拉到 5V，必须移除该上拉并改接 3.3V，或增加双向电平转换器。
...
| I2C0 | PA31/PA28 | 冲突：PA31 为 SW2 用户按键 |
| I2C0 | PB0/PB1 | 冲突：PB0/PB1 为 SW4/SW3 用户按键 |
...
通信参数：**115200-8-N-1，100Hz**。固件上电会设置 100Hz、±4g，并只开启
0x01/0x03 主动上报。帧格式见 `docs/ATK-MS901M/ATK-MS901M模块用户手册_V1.0.pdf`：
...
当前只开启 0x01 和 0x03 时，每周期共 28 字节，100Hz 约 2800B/s，占
115200-8-N-1 有效链路能力约 24%。
...
- `src/middle/hardware/imu_hw.c/h` — UART1 RX + 8×16B DMA 队列
- `src/middle/imu.c/h` — 流式状态机、校验、物理量换算和 100ms 通用缓存检查
- `src/app/imu_app.c/h` — 开启接收并配置主动流；1Hz 限流输出状态

串口调试（UART0）：除 `[hb]` / `[gs]` 外，每 1 s 输出 `[imu]` 行：

`[imu] seq,roll,pitch,yaw,ax,ay,az,bad=N,ovf=N`

姿态角单位为 **°**，加速度单位为 **m/s²**。通用在线状态要求 100ms 内同时
收到 0x01 和 0x03；摆杆前馈只接受 25ms 内的 0x03。等待输出还会显示有效标志、
正确帧数、坏帧数和 DMA 队列溢出数。
...
0.96 寸 **128×64** 单色 OLED，4 针 I2C（控制器 SSD1306/兼容），使用独立的 **硬件 I2C0**（400 kHz，SysConfig 初始化）。原 OLED 的 B0/B1 接线已迁移，两个引脚现用于 SW4/SW3 用户按键。
...
**PB12/PB13 不支持 I2C 硬件复用**。B0/B1 不再连接 OLED 或红外模块，但现已用于 SW4/SW3 用户按键。
...
与现有外设无冲突：OLED 使用 PB17/PB18，红外使用 PA15/PA16，PB0/PB1 用作用户按键。
